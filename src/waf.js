/**
 * WAF module — shared between jcrt-files (files.jcrt.org) Worker instances.
 ...
 *   setCorsHeaders(headers, origin)  → void  (API-style CORS)
 *   TRUSTED_ORIGIN_RE                regex
 *   INDEXING_BOT_RE                  regex
 */

// ─── Trusted domain family ────────────────────────────────────────────────────
// Any subdomain of the four JCRT-family domains may make cross-origin requests.
...
// Path traversal (raw + URL-encoded + double-encoded)
...
// High-confidence SQL injection (academic content rarely triggers these)
const SQLI_RE =
...
// High-confidence XSS / script injection
const XSS_RE =
...
// Shell injection — checked only in query strings
...
/**
 * Inspects a request for attack signatures.
 * Returns { blocked: false } if clean, or { blocked: true, reason, status } if not.
 *
...
/**
 * Sets API-style CORS headers for a trusted cross-origin request.
 * For file-serving CORS (Range / ETag exposure) use the local applyCors()
 * in worker.js instead — that one is tuned for R2 byte-range responses.
 *
